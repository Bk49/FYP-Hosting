        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (level != undefined && level != "")
                    recent_match_opt.level = parseInt(level);
                if (topic != undefined && topic != "")
                    recent_match_opt.topic_name = topic;
                if (skill != undefined && skill != "")
                    recent_match_opt.skill_name = skill;

                var current = await Quiz.find(recent_match_opt)
                    .sort({ _id: -1 })
                    .limit(1);
                if (current.length > 0) {
                    ...
                    result.current.difficult_average_score =
